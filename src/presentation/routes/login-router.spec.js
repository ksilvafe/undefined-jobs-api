...
describe('Login Router', () => {
  ...
  test('Should return 401 when credentials are provider', async () => {
    const { sut, authUseCaseSpy } = makeSut()
  ...
  test('Should return 500 no AuthUseCase is provider', async () => {
  ...
  test('Should return 500 no EmailValidator is provider', async () => {
    const authUseCaseSpy = makeAuthUseCase()
    const sut = new LoginRouter(authUseCaseSpy)
    const httpRequest = {
      body: {
  ...
  test('Should return 500 no EmailValidator has no method', async () => {
    const authUseCaseSpy = makeAuthUseCase()
    const sut = new LoginRouter(authUseCaseSpy, {})
    const httpRequest = {
      body: {
  ...
  test('Should return 500 no EmailValidator throws', async () => {
    const authUseCaseSpy = makeAuthUseCase()
